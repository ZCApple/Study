//
//  UICollectionViewCustom.swift
//  StudyProject
//
//  Created by 路过蜻蜓 on 2022/3/16.
//

import Foundation
import UIKit

class UICollectionViewCustom: UIView {
    ...
    private func setupUI() {
        let layout = UICollectionViewCustomFlowLayout(waterCount: 2)
    ...
    private func refreshItemSize() {
        itemSizeArray.removeAll()
        let width = (kScreenWidth - 16 * 3) / 2
        for dict in dataSource {
            let introl = dict["introl"] ?? ""
            let paraStyle = NSMutableParagraphStyle()
            paraStyle.lineSpacing = 4
            var height = NSString(string: introl).boundingRect(with: CGSize(width: width - 24, height: CGFloat.greatestFiniteMagnitude), options: [.usesFontLeading, .usesLineFragmentOrigin], attributes: [.paragraphStyle: paraStyle, .font: UIFont.systemFont(ofSize: 14)], context: nil).size.height
            height = ceil(height) + 28 + 28 + width
            itemSizeArray.append(CGSize(width: width, height: height))
        }
    }
}

extension UICollectionViewCustom: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewCustomFlowLayoutDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, limitSize: CGSize, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return itemSizeArray[indexPath.item]
    }
    ...
    func collectionView(_ collectionView: UICollectionView, insetForSectionAt section: Int) -> UIEdgeInsets {
        return UIEdgeInsets(top: 16, left: 16, bottom: 34, right: 16)
    }
}
